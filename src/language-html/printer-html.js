...
function concat(parts) {
  ...
  return newParts.length === 0
             ? ""
             : newParts.length === 1 ? newParts[0] : builders.concat(newParts);
}
...
function embed(path, print, textToDoc, options) {
        ...
        indent(concat([
          ...
          textToDoc(node.value, {
            ...
            ...(options.parser === "angular"
                    ...
                    : options.parser === "vue" ? {parser : "__vue_expression"}
  ...
  case "attribute": {
    ...
    if (/^PRETTIER_HTML_PLACEHOLDER_\d+_\d+_IN_JS$/.test(
            options.originalText.slice(node.valueSpan.start.offset,
    ...
    if (options.parser === "lwc") {
      ...
      if (interpolationRegex.test(options.originalText.slice(
              node.valueSpan.start.offset, node.valueSpan.end.offset))) {
        return concat([ node.rawName, "=", node.value ]);
      }
    }
    ...
    const embeddedAttributeValueDoc = printEmbeddedAttributeValue(
        node,
        (code, opts) =>
            // strictly prefer single quote to avoid unnecessary html entity
            // escape
    ...
    if (embeddedAttributeValueDoc) {
      return concat([
        node.rawName,
        '="',
        group(mapDoc(embeddedAttributeValueDoc,
...
function genericPrint(path, options, print) {
  ...
  case "ieConditionalComment": {
     ...
     *     >
     */
    const shouldHugContent = node.children.length === 1 &&
  ...
  case "text": {
    ...
    return fill(normalizeParts(
        [].concat(printOpeningTagPrefix(node, options), getTextValueParts(node),
                  printClosingTagSuffix(node, options))));
  }
  case "docType":
    return concat([
  ...
  case "comment": {
    return concat([
      ...
      concat(replaceEndOfLineWith(
          options.originalText.slice(options.locStart(node),
                                     options.locEnd(node)),
          literalline)),
      printClosingTagSuffix(node, options),
    ]);
  ...
  case "attribute": {
    ...
    return concat([
      ...
      concat([
        "=",
        quote,
        concat(replaceEndOfLineWith(quote === '"'
                                        ? value.replace(/"/g, "&quot;")
                                        : value.replace(/'/g, "&apos;"),
                                    literalline)),
        quote,
...
function printChildren(path, options, print) {
  ...
  return concat(path.map((childPath, childIndex) => {
    const childNode = childPath.getValue();
          ...
          replaceEndOfLineWith(
              options.originalText.slice(
                  options.locStart(child) +
                      (child.prev && needsToBorrowNextOpeningTagStartMarker(
                                         child.prev)
    ...
    return isTextLikeNode(prevNode) && isTextLikeNode(nextNode)
               ? prevNode.isTrailingSpaceSensitive
                     ? prevNode.hasTrailingSpaces
                           ? preferHardlineAsLeadingSpaces(nextNode) ? hardline
                                                                     : line
                           : ""
                     : preferHardlineAsLeadingSpaces(nextNode) ? hardline
                                                               : softline
                     ...
                     : !nextNode.isLeadingSpaceSensitive ||
                               ...
                               (needsToBorrowPrevClosingTagEndMarker(
                                ...
                                needsToBorrowParentClosingTagStartMarker(
                                    prevNode.lastChild.lastChild))
                           ? hardline
                           : nextNode.hasLeadingSpaces ? line : softline;
  }
}
...
function printOpeningTag(path, options, print) {
  ...
  const forceNotToBreakAttrContent =
      node.type === "element" && node.fullName === "script" &&
      node.attrs.length === 1 && node.attrs[0].fullName === "src" &&
      node.children.length === 0;
  return concat([
    printOpeningTagStart(node, options),
        ...
        : concat([
            indent(concat([
              ...
              join(line,
                     ...
                     const hasPrettierIgnoreAttribute =
                         typeof ignoreAttributeData === "boolean"
                             ...
                             : Array.isArray(ignoreAttributeData)
                                   ? (attr) => ignoreAttributeData.includes(
                                         attr.rawName)
                                   : () => false;
                   ...
                   })(node.prev && node.prev.type === "comment" &&
            ...
            (node.firstChild &&
             needsToBorrowParentOpeningTagEndMarker(node.firstChild)) ||
                ...
                : node.isSelfClosing
                      ? forceNotToBreakAttrContent ? " " : line
                      : forceNotToBreakAttrContent ? "" : softline,
          ]),
...
function needsToBorrowNextOpeningTagStartMarker(node) {
   ...
   *     >
   */
  return (node.next && !isTextLikeNode(node.next) && isTextLikeNode(node) &&
          node.isTrailingSpaceSensitive && !node.hasTrailingSpaces);
}
...
function needsToBorrowPrevClosingTagEndMarker(node) {
   ...
   *     ^
   */
  return (node.prev && !isTextLikeNode(node.prev) &&
...
function needsToBorrowLastChildClosingTagEndMarker(node) {
   ...
   *     >
   */
  return (node.lastChild && node.lastChild.isTrailingSpaceSensitive &&
          !node.lastChild.hasTrailingSpaces &&
...
function printEmbeddedAttributeValue(node, originalTextToDoc, options) {
    ...
    const rootNode =
        root.type === "NGRoot"
            ? root.node.type === "NGMicrosyntax" &&
                  ...
                  : root.node
            : root.type === "JsExpressionRoot" ? root.node : root;
  ...
  const printExpand = (doc, canHaveTrailingWhitespace = true) => group(concat([
  ...
  if (options.parser === "vue") {
    ...
    if (isKeyMatched(vueEventBindingPatterns)) {
      const value = getValue();
      return printMaybeHug(isVueEventBindingExpression(value)
                               ? textToDoc(value, {parser : "__js_expression"})
                               : stripTrailingHardline(textToDoc(
                                     value, {parser : "__vue_event_binding"})));
    }
    ...
    if (isKeyMatched(jsExpressionBindingPatterns)) {
      return printMaybeHug(textToDoc(getValue(), {parser : "__js_expression"}));
    }
  }
  ...
  if (options.parser === "angular") {
    ...
    const ngExpressionBindingPatterns = [
      "^\\[.+\\]$",
      "^bind(on)?-",
      // Unofficial rudimentary support for some of the most used directives of
      // AngularJS 1.x
    ...
    if (isKeyMatched(ngI18nPatterns)) {
      ...
      return printExpand(fill(getTextValueParts(node, value)),
                         !value.includes("@@"));
    }
