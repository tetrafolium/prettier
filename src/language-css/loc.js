...
/**
 * Workaround for a bug: quotes in inline comments corrupt loc data of
 * subsequent nodes. This function replaces the quotes with U+FFFE and U+FFFF.
 * Later, when the comments are printed, their content is extracted from the
 * original text or restored by replacing the placeholder characters back with
 * quotes.
 * - https://github.com/prettier/prettier/issues/7780
 * - https://github.com/shellscape/postcss-less/issues/145
 * - About noncharacters (U+FFFE and U+FFFF):
 * http://www.unicode.org/faq/private_use.html#nonchar1
 * @param text {string}
 */
function replaceQuotesInInlineComments(text) {
  /**
   * @typedef { 'initial' | 'single-quotes' | 'double-quotes' | 'url' |
   * 'comment-block' | 'comment-inline' } State
   */
  /** @type {State} */
  let state = "initial";
  /** @type {State} */
