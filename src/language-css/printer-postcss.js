...
function genericPrint(path, options, print) {
  ...
  case "css-comment": {
    ...
    const text = options.originalText.slice(options.locStart(node),
                                            options.locEnd(node));
  ...
  case "css-rule": {
    return concat([
      ...
      node.nodes ? concat([
        node.selector && node.selector.type === "selector-unknown" &&
  ...
  case "css-decl": {
    ...
    return concat([
      ...
      node.extend ? "" : " ",
      hasComposesNode(node) ? removeLines(path.call(print, "value"))
      ...
      node.nodes ? concat([
        " {",
        indent(concat([ softline, printNodeSequence(path, options, print) ])),
        softline,
        "}",
      ])
    ...
    return concat([
      ...
      node.params ? concat([
        ...
        path.call(print, "params"),
                  ...
                  : "",
      node.selector ? indent(concat([ " ", path.call(print, "selector") ]))
                    : "",
      node.value ? group(concat([
        " ",
        path.call(print, "value"),
        isSCSSControlDirectiveNode(node)
            ? hasParensAroundNode(node) ? " " : line
            : "",
      ]))
                 : node.name === "else" ? " " : "",
      node.nodes ? concat([
  ...
  case "selector-tag": {
    ...
    return concat([
      ...
      prevNode.type === "selector-nesting"
          ? node.value
          : adjustNumbers(isKeyframeAtRuleKeywords(path, node.value)
  ...
  case "selector-attribute": {
    return concat([
      ...
      node.value ? quoteAttributeValue(
                       adjustStrings(node.value.trim(), options), options)
                 : "",
      node.insensitive ? " i" : "",
      "]",
    ]);
  ...
  case "selector-combinator": {
    if (node.value === "+" || node.value === ">" || node.value === "~" ||
        node.value === ">>>") {
      const parentNode = path.getParentNode();
      const leading = parentNode.type === "selector-selector" &&
  ...
  case "selector-unknown": {
    ...
    // originalText has to be used for Less, see replaceQuotesInInlineComments
    // in loc.js
    const parentNode = path.getParentNode();
  ...
  case "value-comment": {
    return concat([
      ...
      // value-* nodes don't have correct location data, so we have to rely on
  ...
  case "value-comma_group": {
    ...
    const isGridValue = declAncestorProp && parentNode.type === "value-value" &&
    ...
    for (let i = 0; i < node.groups.length; ++i) {
      ...
      // Ignore spaces before/after string interpolation (i.e.
      // `"#{my-fn("_")}"`)
      ...
      const isEndingSCSSInterpolationInString =
          ...
          iNextNode.type === "value-string" && iNextNode.value.endsWith("}");

      if (isStartSCSSInterpolationInString ||
      ...
      // Ignore `~` in Less (i.e. `content: ~"^//* some horrible but needed css
      // hack";`)
      if (iNode.value === "~") {
        continue;
      }
      ...
      // Ignore escape `\`
      if (iNode.value && iNode.value.includes("\\") && iNextNode &&
          iNextNode.type !== "value-comment") {
        continue;
      }

      // Ignore escaped `/`
      if (iPrevNode && iPrevNode.value &&
      ...
      // Ignore spaces after `#` and after `{` and before `}` in SCSS
      // interpolation (i.e. `#{variable}`)
      if (isHashNode(iNode) || isLeftCurlyBraceNode(iNode) ||
      ...
      // Print spaces before and after math operators beside SCSS interpolation
      // as is (i.e. `#{$var}+5`, `#{$var} +5`, `#{$var}+ 5`, `#{$var} + 5`)
      ...
      // Print spaces before and after addition and subtraction math operators
      // as is in `calc` function due to the fact that it is not valid syntax
      // (i.e. `calc(1px+1px)`, `calc(1px+ 1px)`, `calc(1px +1px)`, `calc(1px +
      // 1px)`)
      if (insideValueFunctionNode(path, "calc") &&
          (isAdditionNode(iNode) || isAdditionNode(iNextNode) ||
           isSubtractionNode(iNode) || isSubtractionNode(iNextNode)) &&
          hasEmptyRawBefore(iNextNode)) {
        continue;
      }

      // Print spaces after `+` and `-` in color adjuster functions as is (e.g.
      // `color(red l(+ 20%))`) Adjusters with signed numbers (e.g. `color(red
      // l(+20%))`) output as-is.
      const isColorAdjusterNode =
          (isAdditionNode(iNode) || isSubtractionNode(iNode)) && i === 0 &&
      ...
      const requireSpaceBeforeOperator =
          ...
          iNode.type === "value-func" || isWordNode(iNode);
      const requireSpaceAfterOperator =
          iNextNode.type === "value-func" || isWordNode(iNextNode) ||
      ...
      if (!(isMultiplicationNode(iNextNode) || isMultiplicationNode(iNode)) &&
          !insideValueFunctionNode(path, "calc") && !isColorAdjusterNode &&
          ((isDivisionNode(iNextNode) && !requireSpaceBeforeOperator) ||
           ...
           isSubtractionNode(iNextNode) || isSubtractionNode(iNode)) &&
          (hasEmptyRawBefore(iNextNode) ||
           (isMathOperator &&
      ...
      if (isControlDirective &&
          (isEqualityOperatorNode(iNextNode) ||
           ...
           isIfElseKeywordNode(iNextNode) || isEachKeywordNode(iNode) ||
      ...
      if (isGridValue) {
        if (iNode.source && iNextNode.source &&
      ...
      // Note: `grip` property have `/` delimiter and it is not math operation,
      // so `grid` property handles above
  ...
  case "value-paren_group": {
    ...
    if (parentNode && isURLFunctionNode(parentNode) &&
            ...
            join(concat([ ",", line ]),
                 path.map(
                     (childPath) => {
                       ...
                       if (isKeyValuePairNode(node) &&
                           node.type === "value-comma_group" && node.groups &&
                           node.groups[2] &&
                           node.groups[2].type === "value-paren_group") {
                         printed.contents.contents.parts[1] =
                             group(printed.contents.contents.parts[1]);
  ...
  case "value-colon": {
    return concat([
      node.value,
      // Don't add spaces on `:` in `url` function (i.e. `url(fbglyph:
      // cross-outline, fig-white)`)
...
function printNodeSequence(path, options, print) {
  ...
  path.map((pathChild) => {
    ...
    if (prevNode && prevNode.type === "css-comment" &&
        prevNode.text.trim() === "prettier-ignore") {
      const childNode = pathChild.getValue();
      parts.push(options.originalText.slice(options.locStart(childNode),
    ...
    if (i !== node.nodes.length - 1) {
      if ((node.nodes[i + 1].type === "css-comment" &&
           !hasNewline(options.originalText,
                       options.locStart(node.nodes[i + 1]),
                       {backwards : true}) &&
           node.nodes[i].type !== "yaml" && node.nodes[i].type !== "toml") ||
        ...
        if (isNextLineEmpty(options.originalText, pathChild.getValue(),
                            options.locEnd) &&
            node.nodes[i].type !== "yaml" && node.nodes[i].type !== "toml") {
          parts.push(hardline);
        }
...
const ADJUST_NUMBERS_REGEX = new RegExp(STRING_REGEX.source + "|" +
...
function adjustNumbers(value) {
  return value.replace(ADJUST_NUMBERS_REGEX,
