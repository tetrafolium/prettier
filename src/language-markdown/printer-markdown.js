...
function genericPrint(path, options, print) {
  ...
  if (shouldRemainTheSameContent(path)) {
    return concat(
        splitText(options.originalText.slice(node.position.start.offset,
                                             node.position.end.offset),
                  options)
            .map((node) => node.type === "word"
                               ? node.value
                               : node.value === ""
                                     ? ""
    ...
    return node.value
        ...
        .replace(new RegExp(
                 ...
                 (_, text1, underscore1, underscore2, text2) =>
                     (underscore1 ? `${text1}${underscore1}`
                                  : `${underscore2}${text2}`)
                         .replace(/_/g,
                                  "\\_")); // escape all `_` except concating
                                           // with non-punctuation, e.g. `1_2_3`
                                           // is not considered emphasis
  case "whitespace": {
  ...
  case "emphasis": {
    ...
    const nextNode = parentNode.children[index + 1];
    const hasPrevOrNextWord = // `1*2*3` is considered emphasis but `1_2_3` is
                              // not
        (prevNode && prevNode.type === "sentence" &&
         prevNode.children.length > 0 &&
         privateUtil.getLast(prevNode.children).type === "word" &&
         !privateUtil.getLast(prevNode.children).hasTrailingPunctuation) ||
        (nextNode && nextNode.type === "sentence" &&
         nextNode.children.length > 0 && nextNode.children[0].type === "word" &&
  ...
  case "inlineCode": {
    const backtickCount =
        privateUtil.getMinNotPresentContinuousCount(node.value, "`");
  ...
  case "link":
    ...
    case "<": {
      ...
      const url =
          ...
          node.url.startsWith(mailto) &&
                  options.originalText.slice(node.position.start.offset + 1,
                                             node.position.start.offset + 1 +
                                                 mailto.length) !== mailto
    ...
    default:
      return options.originalText.slice(node.position.start.offset,
                                        node.position.end.offset);
    }
  case "image":
    return concat([
  ...
  case "code": {
    ...
    const style = styleUnit.repeat(Math.max(
        3, privateUtil.getMaxContinuousCount(node.value, styleUnit) + 1));
    return concat([
      style,
      node.lang || "",
      hardline,
      concat(replaceEndOfLineWith(
          getFencedCodeBlockValue(node, options.originalText), hardline)),
      hardline,
      style,
    ]);
  }
  ...
  case "list": {
    ...
    return printChildren(path, options, print, {
      processor : (childPath, index) => {
        ...
        if (childNode.children.length === 2 &&
        ...
        function getPrefix() {
          ...
          return node.isAligned ||
                         /* workaround for
                            https://github.com/remarkjs/remark/issues/315 */
  ...
  case "thematicBreak": {
    ...
    const nthSiblingIndex = getNthListSiblingIndex(
        path.getParentNode(counter), path.getParentNode(counter + 1));
  ...
  case "footnoteDefinition": {
    ...
    const shouldInlineFootnote = node.children.length === 1 &&
    ...
    printChildren(path, options, print, {
      processor : (childPath, index) => {
        ...
        const alignment = " ".repeat(
            clamp(options.tabWidth - listPrefix.length, 0,
                  3) // 4+ will cause indented code block
        );
...
function alignListPrefix(prefix, options) {
  ...
  return (prefix +
          " ".repeat(
              additionalSpaces >= 4
                  ? 0
                  : additionalSpaces // 4+ will cause indented code block
              ));
...
function getNthListSiblingIndex(node, parentNode) {
  return getNthSiblingIndex(
      node, parentNode, (siblingNode) => siblingNode.ordered === node.ordered);
}
...
function printLine(path, value, options) {
  ...
  const isBreakable = options.proseWrap === "always" &&
...
function printTable(path, options, print) {
  ...
  const columnMaxWidths = contents.reduce(
      (currentWidths, rowContents) => currentWidths.map(
          (width, columnIndex) => Math.max(
              width, privateUtil.getStringWidth(rowContents[columnIndex]))),
      contents[0].map(() => 3) // minimum width = 3 (---, :--, :-:, --:)
  );
  const alignedTable = join(hardlineWithoutBreakParent, [
    printRow(contents[0]),
    printSeparator(),
    join(hardlineWithoutBreakParent,
  ...
  const compactTable = join(hardlineWithoutBreakParent, [
    ...
    join(hardlineWithoutBreakParent,
         contents.slice(1).map(
             (rowContents) => printRow(rowContents, /* isCompact */ true))),
  ]);
  ...
  function printSeparator(isCompact) {
    return concat([
      "| ",
      join(" | ", columnMaxWidths.map((width, index) => {
  ...
  function printRow(rowContents, isCompact) {
    return concat([
      "| ",
      join(" | ", isCompact ? rowContents
                            : rowContents.map((rowContent, columnIndex) => {
                                switch (node.align[columnIndex]) {
                                case "right":
                                  return alignRight(
                                      rowContent, columnMaxWidths[columnIndex]);
                                case "center":
                                  return alignCenter(
                                      rowContent, columnMaxWidths[columnIndex]);
                                default:
      ...
      if (ignoreRanges.length !== 0) {
        ...
        if (index === ignoreRange.start.index) {
          return concat([
            ...
            options.originalText.slice(ignoreRange.start.offset,
...
function isPrettierIgnore(node) {
  ...
  const match =
      node.value.match(/^<!--\s*prettier-ignore(?:-(start|end))?\s*-->$/);
...
function shouldNotPrePrintHardline(node, data) {
  ...
  const isInlineHTML = node.type === "html" &&
...
function shouldPrePrintDoubleHardline(node, data) {
  ...
  const isBlockHtmlWithoutBlankLineBetweenPrevHtml =
      node.type === "html" && data.prevNode && data.prevNode.type === "html" &&
      data.prevNode.position.end.line + 1 === node.position.start.line;

  const isHtmlDirectAfterListItem =
      node.type === "html" && data.parentNode.type === "listItem" &&
      data.prevNode && data.prevNode.type === "paragraph" &&
      data.prevNode.position.end.line + 1 === node.position.start.line;

  return (isPrevNodeLooseListItem ||
...
function shouldRemainTheSameContent(path) {
  ...
  return (ancestorNode && (ancestorNode.type !== "linkReference" ||
...
function clean(ast, newObj, parent) {
  ...
  // for codeblock
  if (ast.type === "code" || ast.type === "yaml" || ast.type === "import" ||
      ast.type === "export" || ast.type === "jsx") {
    delete newObj.value;
  }
  ...
  // for insert pragma
  if (parent && parent.type === "root" && parent.children.length > 0 &&
      (parent.children[0] === ast || ((parent.children[0].type === "yaml" ||
                                       parent.children[0].type === "toml") &&
                                      parent.children[1] === ast)) &&
      ast.type === "html" && pragma.startWithPragma(ast.value)) {
    return null;
  }
}
