...
function embed(path, print, textToDoc, options) {
  ...
  if (node.type === "code" && node.lang !== null) {
    // only look for the first string so as to support
    ...
    if (parser) {
      ...
      const doc = textToDoc(getFencedCodeBlockValue(node, options.originalText),
                            {parser});
  ...
  function getParserName(lang) {
    ...
    const language = supportInfo.languages.find(
        (language) => language.name.toLowerCase() === lang ||
