...
/**
 * based on [MDAST](https://github.com/syntax-tree/mdast) with following
 * modifications:
 *
...
function htmlToJsx() {
  return (ast) => mapAst(ast, (node, _index, [ parent ]) => {
           if (node.type !== "html" || node.value.match(mdx.COMMENT_REGEX) ||
...
function liquid() {
  ...
  tokenizer.locator = function(
      value, fromIndex) { return value.indexOf("{", fromIndex); };
}
