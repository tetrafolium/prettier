 ...
 * @param {object} param0
 * @param {(string | object)[]=} param0.plugins Strings are resolved by
 *     `withPlugins`.
...
function getSupportInfo({
  ...
  const options =
      arrayify(Object.assign({}, ...plugins.map(({options}) => options),
                             coreOptions),
               "name")
          ...
          .map((option) => {
            ...
            if (Array.isArray(option.default)) {
              option.default = option.default.length === 1
                                   ...
                                   : option.default.filter(filterSince)
                                         .sort((info1, info2) => semver.compare(
                                                   info2.since, info1.since))[0]
                                         .value;
            }
  ...
  const languages =
      plugins.reduce((all, plugin) => all.concat(plugin.languages || []), [])
          .filter(filterSince);
