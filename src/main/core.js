...
function ensureAllCommentsPrinted(astComments) {
  ...
  astComments.forEach((comment) => {
    if (!comment.printed) {
      throw new Error('Comment "' + comment.value.trim() +
...
function coreFormat(text, opts, addAlignmentSize) {
  ...
  const result = printDocToString(
      opts.endOfLine === "lf"
          ? doc
          : mapDoc(doc, (currentDoc) => typeof currentDoc === "string" &&
                                                currentDoc.includes("\n")
                                            ? currentDoc.replace(/\n/g, eol)
                                            : currentDoc),
      opts);
  ...
  // Remove extra leading indentation as well as the added indentation after
  // last newline
  ...
  if (opts.cursorOffset >= 0) {
    ...
    if (opts.cursorNode && result.cursorNodeText) {
      ...
      oldCursorNodeText =
          text.slice(oldCursorNodeStart, opts.locEnd(opts.cursorNode));
    ...
    oldCursorNodeCharArray.splice(cursorOffsetRelativeToOldCursorNode, 0,
                                  CURSOR);

    const newCursorNodeCharArray = newCursorNodeText.split("");

    const cursorNodeDiff =
        diff.diffArrays(oldCursorNodeCharArray, newCursorNodeCharArray);
...
function formatRange(text, opts) {
  ...
  const rangeStart2 =
      Math.min(rangeStart, text.lastIndexOf("\n", rangeStart) + 1);
  const indentString = text.slice(rangeStart2, rangeStart);

  const alignmentSize =
      privateUtil.getAlignmentSize(indentString, opts.tabWidth);

  const rangeResult = coreFormat(rangeString, {
    ...opts,
    rangeStart : 0,
    rangeEnd : Infinity,
    ...
    if (cursorOffset >= 0) {
      ...
      while (partIndex < parts.length) {
        ...
        if (partOffset < part.length) {
          parts[partIndex] = parts[partIndex].slice(0, partOffset) +
...
function format(text, opts) {
  ...
  const result =
      hasRangeStart || hasRangeEnd
          ...
          : coreFormat(opts.insertPragma && opts.printer.insertPragma &&
                               !hasPragma
...
module.exports = {
  ...
  printDocToString(
      doc, opts) { return printDocToString(doc, normalizeOptions(opts)); },
};
