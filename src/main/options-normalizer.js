...
function normalizeOptions(options, optionInfos,
  ...
  const unknown = !passThrough
                      ...
                      : Array.isArray(passThrough)
                            ? (key, value) => !passThrough.includes(key)
                                                  ? undefined
                                                  : {[key] : value}
                            : (key, value) => ({[key] : value});
...
function optionInfoToSchema(optionInfo, {isCLI, optionInfos}) {
  ...
  // allow CLI overriding, e.g., prettier package.json --tab-width 1 --tab-width
  // 2
  if (isCLI && !optionInfo.array) {
    const originalPreprocess = parameters.preprocess || ((x) => x);
    parameters.preprocess = (value, schema, utils) => schema.preprocess(
        originalPreprocess(Array.isArray(value) ? value[value.length - 1]
                                                : value),
        utils);
  }
