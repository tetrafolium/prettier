...
function preprocess(ast) { return mapNode(ast, defineShortcuts); }
...
function genericPrint(path, options, print) {
  ...
  const nextEmptyLine = isNode(node,
                               [
                                 "mapping",
                                 "sequence",
                                 "comment",
  ...
  return concat([
    ...
    hasEndComments(node) && !isNode(node, [ "documentHead", "documentBody" ])
        ? align(node.type === "sequenceItem" ? 2 : 0, concat([
...
function _print(node, parentNode, path, options, print) {
                 ...
                 return concat([
                   ...
                   shouldPrintDocumentEndMarker(document, nextDocument)
                       ? concat([
                           hardline,
                           "...",
                           hasTrailingComment(document) ? concat([
                             " ", path.call(print, "trailingComment")
                           ])
                                                        : "",
                         ])
      ...
      node.children.length === 0 ||
              ((lastDescendantNode) =>
                   isNode(lastDescendantNode,
                          [ "blockLiteral", "blockFolded" ]) &&
                   lastDescendantNode.chomping ===
                       "keep")(getLastDescendantNode(node))
          ? ""
          : hardline,
    ]);
  case "document": {
    ...
    return join(hardline, [
      shouldPrintDocumentHeadEndMarker(node, nextDocument, parentNode,
                                       options) === "head"
          ? join(hardline,
                 [
                   ...
                   concat([
                     "---",
                     hasTrailingComment(node.head) ? concat([
                       " ",
  ...
  case "documentHead":
    return join(hardline, [].concat(path.map(print, "children"),
            ...
            : ((lastDescendantNode) =>
                   isNode(lastDescendantNode, [ "blockFolded", "blockLiteral" ])
                       ? lastDescendantNode.chomping === "keep"
                             ? // there's already a newline printed at the end
                               // of blockValue (chomping=keep,
                               // lastDescendant=true)
                             ""
  ...
  case "quoteSingle": {
    ...
    const raw = options.originalText.slice(node.position.start.offset + 1,
                                           node.position.end.offset - 1);

    if ((node.type === "quoteSingle" && raw.includes("\\")) ||
      ...
      return concat([
        singleQuote,
        printFlowScalarContent(node.type,
                               node.type === "quoteDouble"
                                   ? raw
                                         // double quote needs to be escaped by
                                         // backslash in quoteDouble
      ...
      return concat([
        ...
        printFlowScalarContent(node.type,
                               node.type === "quoteSingle"
                                   ? // single quote needs to be escaped by 2
                                     // single quotes in quoteSingle
  ...
  case "blockLiteral": {
    ...
    return concat([
      ...
      hasIndicatorComment(node)
          ...
          : "",
      (node.indent === null ? dedent : dedentToRoot)(align(
          node.indent === null ? options.tabWidth
                               : node.indent - 1 + parentIndent,
          concat(getBlockValueLineContents(node, {
                   parentIndent,
                   isLastDescendant,
                   options,
                 })
                     .reduce((reduced, lineWords, index, lineContents) =>
                                 reduced.concat(
                                     index === 0 ? hardline : "",
  ...
  case "flowMappingItem": {
    ...
    if (isEmptyMappingValue) {
      return node.type === "flowMappingItem" &&
                 ...
                 : node.type === "mappingItem" &&
                           isAbsolutelyPrintedAsSingleLineNode(node.key.content,
                                                               options) &&
    ...
    return forceExplicitKey ? concat([
      ...
      align(2, key),
      hardline,
      join("", path.map(print, "value", "leadingComments")
      ...
      align(2, value),
               ...
               isSingleLineNode(node.key.content) &&
                       ...
                       isAbsolutelyPrintedAsSingleLineNode(node.value.content,
                                                           options)
                   ? concat([
                       key,
                       needsSpaceInFrontOfMappingValue(node) ? " " : "",
                   ...
                   : conditionalGroup([
                       concat([
                         group(concat([
                           ifBreak("? "), group(align(2, key), {id : groupId})
                         ])),
                         ifBreak(
  ...
  case "flowSequence": {
    ...
    const bracketSpacing = node.type === "flowMapping" &&
                                   node.children.length !== 0 &&
    ...
    return concat([
      ...
      indent(concat([
        bracketSpacing,
        concat(path.map(
            (childPath, index) => concat([
              print(childPath),
              index === node.children.length - 1 ? "" : concat([
                ",",
                line,
...
function align(n, doc) {
  return typeof n === "number" && n > 0 ? docBuilders.align(" ".repeat(n), doc)
...
function shouldPrintDocumentHeadEndMarker(document, nextDocument, root,
                                          options) {
  if (
      /**
       * ---
      ...
      (root.children[0] === document &&
       /---(\s|$)/.test(options.originalText.slice(
           options.locStart(document), options.locStart(document) + 4))) ||
      /**
       * %DIRECTIVE
       * ---
       */
...
function isAbsolutelyPrintedAsSingleLineNode(node, options) {
  ...
  if (
      // backslash-newline
      /\\$/m.test(options.originalText.slice(node.position.start.offset,
                                             node.position.end.offset))) {
    return false;
  }
