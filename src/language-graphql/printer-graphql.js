...
function genericPrint(path, options, print) {
  ...
  case "OperationDefinition": {
    ...
    return concat([
      ...
      n.variableDefinitions && n.variableDefinitions.length ? group(concat([
        "(",
        indent(concat([
          softline,
          join(concat([ ifBreak("", ", "), softline ]),
  ...
  case "SelectionSet": {
    return concat([
      ...
      indent(concat([
        hardline,
        join(hardline, path.call((selectionsPath) => printSequence(
                                     selectionsPath, options, print),
                                 "selections")),
      ])),
      hardline,
      "}",
  ...
  case "Field": {
    return group(concat([
      ...
      path.call(print, "name"),
      n.arguments.length > 0 ? group(concat([
        "(",
        indent(concat([
          softline,
          join(concat([ ifBreak("", ", "), softline ]),
               path.call((argsPath) => printSequence(argsPath, options, print),
                         "arguments")),
        ])),
        softline,
        ")",
  ...
  case "ObjectTypeDefinition": {
    return concat([
      ...
      "type ",
      path.call(print, "name"),
      n.interfaces.length > 0 ? concat([
        " implements ",
        concat(printInterfaces(path, options, print)),
      ])
                              : "",
      printDirectives(path, print, n),
      n.fields.length > 0 ? concat([
        " {",
        indent(concat([
          hardline,
          join(hardline, path.call((fieldsPath) => printSequence(
                                       fieldsPath, options, print),
                                   "fields")),
        ])),
        hardline,
        "}",
  ...
  case "EnumTypeDefinition": {
    return concat([
      ...
      n.values.length > 0 ? concat([
        " {",
        indent(concat([
          hardline,
          join(hardline, path.call((valuesPath) => printSequence(
                                       valuesPath, options, print),
                                   "values")),
        ])),
        hardline,
        "}",
  ...
  case "SchemaDefinition": {
    return concat([
      ...
      printDirectives(path, print, n),
      " {",
      n.operationTypes.length > 0 ? indent(concat([
        hardline,
        join(hardline,
             path.call((opsPath) => printSequence(opsPath, options, print),
                       "operationTypes")),
      ]))
                                  : "",
      hardline,
  ...
  case "InlineFragment": {
    return concat([
      "...",
      n.typeCondition ? concat([ " on ", path.call(print, "typeCondition") ])
                      : "",
      printDirectives(path, print, n),
      " ",
      path.call(print, "selectionSet"),
...
function printSequence(sequencePath, options, print) {
  ...
  return sequencePath.map((path, i) => {
    ...
    if (isNextLineEmpty(options.originalText, path.getValue(),
                        options.locEnd) &&
        i < count - 1) {
...
function canAttachComment(node) { return node.kind && node.kind !== "Comment"; }
...
function determineInterfaceSeparatorBetween(first, second, options) {
  const textBetween =
      options.originalText.slice(first.loc.end, second.loc.start)
          .replace(/#.*/g, "")
          .trim();
...
function printInterfaces(path, options, print) {
  ...
  for (let index = 0; index < interfaces.length; index++) {
    ...
    if (index > 0) {
      parts.push(determineInterfaceSeparatorBetween(interfaces[index - 1],
                                                    interfaceNode, options));
    }
