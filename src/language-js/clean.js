...
function clean(ast, newObj, parent) {
  ...
  if (ast.type === "JSXExpressionContainer" &&
      ast.expression.type === "Literal" && ast.expression.value === " ") {
    return null;
  }

  // (TypeScript) Ignore `static` in `constructor(static p) {}`
  // and `export` in `constructor(export p) {}`
  if (ast.type === "TSParameterProperty" && ast.accessibility === null &&
      !ast.readonly) {
    return {
      type : "Identifier",
  ...
  if (ast.type === "TSNamespaceExportDeclaration" && ast.specifiers &&
  ...
  if ((ast.type === "Property" || ast.type === "ObjectProperty" ||
       ast.type === "MethodDefinition" || ast.type === "ClassProperty" ||
       ast.type === "TSPropertySignature" ||
       ast.type === "ObjectTypeProperty") &&
      typeof ast.key === "object" && ast.key &&
      (ast.key.type === "Literal" || ast.key.type === "StringLiteral" ||
  ...
  // styled-jsx
  if (ast.type === "JSXElement" && ast.openingElement.name.name === "style" &&
  ...
  if (ast.type === "JSXAttribute" && ast.name.name === "css" &&
  ...
  if (ast.type === "Decorator" && expression.type === "CallExpression" &&
  ...
  if (ast.type === "TaggedTemplateExpression" &&
      (ast.tag.type === "MemberExpression" ||
       (ast.tag.type === "Identifier" &&
        (ast.tag.name === "gql" || ast.tag.name === "graphql" ||
         ast.tag.name === "css" || ast.tag.name === "md" ||
         ast.tag.name === "markdown" || ast.tag.name === "html")) ||
  ...
  if (ast.type === "TemplateLiteral") {
    ...
    // we will not trim the comment value and we will expect exactly one space
    // on either side of the GraphQL string Also see ./embed.js
    const hasLanguageComment =
        ast.leadingComments &&
        ast.leadingComments.some(
            (comment) => comment.type === "CommentBlock" && [
              "GraphQL", "HTML"
            ].some((languageName) => comment.value === ` ${languageName} `));
    if (hasLanguageComment || (parent.type === "CallExpression" &&
