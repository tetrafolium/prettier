...
function postprocess(ast, options) {
  // Keep Babel's non-standard ParenthesizedExpression nodes only if they have
  // Closure-style type cast comments.
  if (options.parser !== "typescript" && options.parser !== "flow") {
    const startOffsetsOfTypeCastedNodes = new Set();

    // Comments might be attached not directly to ParenthesizedExpression but to
    // its ancestor. E.g.: /** @type {Foo} */ (foo).bar(); Let's use the fact
    // that those ancestors and ParenthesizedExpression have the same start
    // offset.
  ...
  visitNode(ast, (node) => {
    ...
    case "SequenceExpression":
      // Babel (unlike other parsers) includes spaces and comments in the range.
      // Let's unify this.
      ...
      break;
    case "ClassProperty":
      // TODO: Temporary auto-generated node type. To remove when
      // typescript-estree has proper support for private fields.
      if (node.key && node.key.type === "TSPrivateIdentifier" &&
          getNextNonSpaceNonCommentCharacter(options.originalText, node.key,
                                             locEnd) === "?") {
        node.optional = true;
      }
      break;
   ...
   * - do nothing if there's a semicolon on `toOverrideNode.end` (no need to
   * fix)
   */
