...
// TODO(azz): anything that imports from main shouldn't be in a `language-*`
// dir.
...
function genericPrint(path, options, printPath, args) {
  ...
  if (node.type === "ClassMethod" || node.type === "ClassPrivateMethod" ||
  ...
  } else if (node.decorators && node.decorators.length > 0 &&
    ...
    const shouldBreak = node.type === "ClassExpression" ||
  ...
  } else if (isExportDeclaration(node) && node.declaration &&
             node.declaration.decorators &&
             node.declaration.decorators.length > 0 &&
             // Only print decorators here if they were written before the
             // export, otherwise they are printed by the node.declaration
 ...
 * @property {boolean} shouldCheckJsx - Whether to check for and print in JSX
 * mode.
 * @property {string} conditionalNodeType - The type of the conditional
 * expression node, ie "ConditionalExpression" or "TSConditionalType".
 ...
 * @param {FastPath} path - The path to the
 *     ConditionalExpression/TSConditionalType node.
...
function printTernaryOperator(path, options, print, operatorOptions) {
  ...
  const isParentTest = parent.type === operatorOptions.conditionalNodeType &&
                       operatorOptions.testNodePropertyNames.some(
                           (prop) => parent[prop] === node);
  let forceNoIndent =
  ...
  if (operatorOptions.shouldCheckJsx &&
      (isJSXNode(node[operatorOptions.testNodePropertyNames[0]]) ||
       isJSXNode(consequentNode) || isJSXNode(alternateNode) ||
  ...
  const result = maybeGroup(concat([].concat(
      ((testDoc) =>
           /**
            *     a
            *       ? b
...
function printPathNoParens(path, options, print, args) {
  ...
  case "File":
    ...
    if (n.program && n.program.interpreter) {
      parts.push(path.call(
          (programPath) => programPath.call(print, "interpreter"), "program"));
    }
  ...
  case "Program":
    ...
    if (n.directives) {
      path.each((childPath) => {
        ...
        if (isNextLineEmpty(options.originalText, childPath.getValue(),
                            options.locEnd)) {
          parts.push(hardline);
        }
      }, "directives");
  ...
  case "ExpressionStatement":
    ...
    if (options.parser === "__vue_event_binding") {
      const parent = path.getParentNode();
      if (parent.type === "Program" && parent.body.length === 1 &&
          parent.body[0] === n) {
        return concat([
  ...
  // Babel non-standard node. Used for Closure-style type casts. See
  // postprocess.js.
  ...
  case "AssignmentExpression":
    return printAssignment(n.left, path.call(print, "left"),
                           concat([ " ", n.operator ]), n.right,
                           path.call(print, "right"), options);
  ...
  case "NGPipeExpression": {
    ...
    const parts =
        printBinaryishExpressions(path, print, options,
                                  /* isNested */ false, isInsideParenthesis);

    //   if (
    //     this.hasPlugin("dynamicImports") && this.lookahead().type ===
    //     tt.parenLeft
    //   ) {
    //
    ...
    // Avoid indenting sub-expressions in some cases where the first
    // sub-expression is already indented accordingly. We should indent
    // sub-expressions where the first case isn't indented.
    ...
    const chain = group(concat([
                          // Don't include the initial expression in the
                          // indentation level. The first item is guaranteed to
                          // be the first left-most expression.
  ...
  case "TSTypeAssertion": {
    const shouldBreakAfterCast = !(n.expression.type === "ArrayExpression" ||
                                   n.expression.type === "ObjectExpression");

    const castGroup = group(concat([
      "<",
  ...
  case "BindExpression":
    ...
    parts.push(group(indent(concat(
        [ softline, printBindExpressionCallee(path, options, print) ]))));
  ...
  case "ArrowFunctionExpression": {
    ...
    const dangling = comments.printDanglingComments(
        path, options,
        /* sameIndent */ true, (comment) => {
          const nextCharacter = getNextNonSpaceNonCommentCharacterIndex(
              options.originalText, comment, options.locEnd);
          return (options.originalText.slice(nextCharacter,
                                             nextCharacter + 2) === "=>");
        });
    if (dangling) {
      parts.push(" ", dangling);
    ...
    if (!hasLeadingOwnLineComment(options.originalText, n.body, options) &&
        (n.body.type === "ArrayExpression" ||
         ...
         n.body.type === "BlockStatement" || isJSXNode(n.body) ||
    ...
    if (n.body.type === "SequenceExpression") {
      return group(concat([
        concat(parts),
        group(concat(
            [ " (", indent(concat([ softline, body ])), softline, ")" ])),
      ]));
    }
  ...
  case "ImportDeclaration": {
    ...
    if (n.specifiers && n.specifiers.length > 0) {
      ...
      parts.push(" from ");
               ...
               // import {} from 'x'
               /{\s*}/.test(options.originalText.slice(
                   options.locStart(n), options.locStart(n.source)))) {
      parts.push("{} from ");
    }
  ...
  case "BlockStatement": {
    ...
    if (!hasContent && !hasDirectives && !hasDanglingComments(n) &&
        (parent.type === "ArrowFunctionExpression" ||
         ...
         parent.type === "DoWhileStatement" || parent.type === "DoExpression" ||
  ...
  case "CallExpression": {
    ...
    if (
        ...
        (!isNew && n.callee.type === "Identifier" &&
    ...
    // Inline Flow annotation comments following Identifiers in Call nodes need
    // to stay with the Identifier. For example:
    //
    // foo /*:: <SomeGeneric> */(bar);
    //
    // Here, we ensure that such comments stay between the Identifier and the
    // Callee.
  ...
  case "TSInterfaceDeclaration":
    ...
    parts.push(n.abstract ? "abstract " : "",
               printTypeScriptModifiers(path, options, print), "interface ",
               path.call(print, "id"),
  ...
  case "TSTypeLiteral": {
    ...
    const firstProperty =
        fields.map((field) => n[field][0])
            .sort((a, b) => options.locStart(a) - options.locStart(b))[0];

    const parent = path.getParentNode(0);
    const isFlowInterfaceLikeBody = isTypeAnnotation && parent &&
                                    (parent.type === "InterfaceDeclaration" ||
                                     parent.type === "DeclareInterface" ||
                                     parent.type === "DeclareClass") &&
                                    path.getName() === "body";
    const shouldBreak =
        n.type === "TSInterfaceBody" || isFlowInterfaceLikeBody ||
        (n.type === "ObjectPattern" && parent.type !== "FunctionDeclaration" &&
         parent.type !== "FunctionExpression" &&
         parent.type !== "ArrowFunctionExpression" &&
         parent.type !== "ObjectMethod" && parent.type !== "ClassMethod" &&
         parent.type !== "ClassPrivateMethod" &&
         parent.type !== "AssignmentPattern" && parent.type !== "CatchClause" &&
         n.properties.some((property) =>
                               property.value &&
                               (property.value.type === "ObjectPattern" ||
                                property.value.type === "ArrayPattern"))) ||
        (n.type !== "ObjectPattern" && firstProperty &&
         hasNewlineInRange(options.originalText, options.locStart(n),
    ...
    const props = propsAndLoc.sort((a, b) => a.loc - b.loc).map((prop) => {
      ...
      if ((prop.node.type === "TSPropertySignature" ||
           prop.node.type === "TSMethodSignature" ||
           prop.node.type === "TSConstructSignatureDeclaration") &&
          hasNodeIgnoreComment(prop.node)) {
        separatorParts.shift();
      }
      if (isNextLineEmpty(options.originalText, prop.node, options.locEnd)) {
    ...
    const canHaveTrailingSeparator =
        !(lastElem && (lastElem.type === "RestProperty" ||
                       lastElem.type === "RestElement" ||
                       hasNodeIgnoreComment(lastElem) || n.inexact));
    ...
    if (props.length === 0) {
      ...
      content = concat([
        ...
        ifBreak(canHaveTrailingSeparator &&
    ...
    // type
    if (path.match((node) => node.type === "ObjectPattern" && !node.decorators,
                   (node, name, number) =>
                       shouldHugArguments(node) &&
                       (name === "params" || name === "parameters") &&
                       number === 0) ||
        path.match(shouldHugType, (node, name) => name === "typeAnnotation",
  ...
  case "Property":
    ...
    if (n.shorthand) {
      ...
      parts.push(printAssignment(n.key, printPropertyKey(path, options, print),
                                 ":", n.value, path.call(print, "value"),
                                 options));
    }
  ...
  case "ArrayPattern":
    if (n.elements.length === 0) {
      ...
      const shouldBreak =
          n.elements.length > 1 && n.elements.every((element, i, elements) => {
            const elementType = element && element.type;
            if (elementType !== "ArrayExpression" &&
      ...
      parts.push(
          group(concat([
                  ...
                  ifBreak(canHaveTrailingComma && !needsForcedTrailingComma &&
  ...
  case "Literal": {
    ...
    // TypeScript workaround for
    // https://github.com/JamesHenry/typescript-estree/issues/2 See
    // corresponding workaround in needs-parens.js
    const grandParent = path.getParentNode(1);
    const isTypeScriptDirective = options.parser === "typescript" &&
                                  typeof n.value === "string" && grandParent &&
  ...
  case "VariableDeclaration": {
    const printed =
        path.map((childPath) => { return print(childPath); }, "declarations");
    ...
    parts = [
      ...
      indent(concat(printed.slice(1).map(
          (p) => concat(
  ...
  case "TSTypeAliasDeclaration": {
    ...
    const printed = printAssignmentRight(
        n.id, n.typeAnnotation,
        n.typeAnnotation && path.call(print, "typeAnnotation"), options);

    parts.push("type ", path.call(print, "id"),
               path.call(print, "typeParameters"), " =", printed, semi);

    return group(concat(parts));
  }
  case "VariableDeclarator":
    return printAssignment(n.id, path.call(print, "id"), " =", n.init,
                           n.init && path.call(print, "init"), options);
  case "WithStatement":
    return group(concat([
      "with (",
      path.call(print, "object"),
  ...
  case "IfStatement": {
    ...
    if (n.alternate) {
      ...
      parts.push("else",
                 group(adjustClause(n.alternate, path.call(print, "alternate"),
  ...
  case "CatchClause":
    if (n.param) {
      ...
      return concat([
        ...
        hasComments
            ? concat(
                  [ "(", indent(concat([ softline, param ])), softline, ") " ])
            : concat([ "(", param, ") " ]),
        path.call(print, "body"),
      ]);
    }
  ...
  case "SwitchStatement":
    return concat([
      ...
      " {",
      n.cases.length > 0 ? indent(concat([
        hardline,
        join(hardline, path.map(
                           (casePath) => {
  ...
  case "SwitchCase": {
    ...
    const consequent =
        n.consequent.filter((node) => node.type !== "EmptyStatement");
    ...
    if (consequent.length > 0) {
      ...
      parts.push(consequent.length === 1 &&
  ...
  case "JSXExpressionContainer": {
    ...
    const shouldInline =
        ...
        (!hasComments && (n.expression.type === "ArrayExpression" ||
                          n.expression.type === "ObjectExpression" ||
  ...
  case "JSXElement": {
    const elem = comments.printComments(
        path, () => printJSXElement(path, options, print), options);
  ...
  case "JSXOpeningElement": {
    ...
    const nameHasComments =
        ...
        (n.typeParameters && n.typeParameters.comments &&
    ...
    const shouldBreak =
        n.attributes &&
        n.attributes.some((attr) => attr.value && isStringLiteral(attr.value) &&
    ...
    return group(
        concat([
          ...
          path.call(print, "typeParameters"),
          concat([
            indent(concat(path.map((attr) => concat([ line, print(attr) ]),
                                   "attributes"))),
  ...
  case "ClassPrivateProperty": {
    ...
    parts.push(printPropertyKey(path, options, print), printOptionalToken(path),
               printTypeAnnotation(path, options, print));
    if (n.value) {
      parts.push(" =", printAssignmentRight(
                           n.key, n.value, path.call(print, "value"), options));
    }
  ...
  case "TemplateLiteral": {
    ...
    if (isSimple) {
      expressions = expressions.map(
          (doc) => printDocToString(doc, {...options, printWidth : Infinity})
                       .formatted);
    }
    ...
    path.each((childPath) => {
      ...
      if (i < expressions.length) {
        ...
        if (!isSimple) {
          // Breaks at the template element boundaries (${ and }) are preferred
          // to breaking in the middle of a MemberExpression
          if ((n.expressions[i].comments && n.expressions[i].comments.length) ||
  ...
  case "TupleTypeAnnotation": {
    ...
    const hasRest = n[typesField].length > 0 &&
  ...
  case "OpaqueType": {
    parts.push("opaque type ", path.call(print, "id"),
               path.call(print, "typeParameters"));
  ...
  case "TSFunctionType": {
    ...
    let isArrowFunctionTypeAnnotation =
        ...
        !(((parent.type === "ObjectTypeProperty" ||
           ...
           !getFlowVariance(parent) && !parent.optional &&
    ...
    // around :(
    const needsParens = needsColon && isArrowFunctionTypeAnnotation &&
    ...
    if (n.returnType || n.predicate || n.typeAnnotation) {
      parts.push(isArrowFunctionTypeAnnotation ? " => " : ": ",
                 path.call(print, "returnType"), path.call(print, "predicate"),
                 path.call(print, "typeAnnotation"));
    }
    if (needsParens) {
      parts.push(")");
  ...
  case "UnionTypeAnnotation": {
    ...
    if ((parent.type === "TupleTypeAnnotation" && parent.types.length > 1) ||
  ...
  case "TypeAlias": {
    ...
    const printed =
        printAssignmentRight(n.id, n.right, path.call(print, "right"), options);
  ...
  case "TypeParameterInstantiation": {
    ...
    const commentStart =
        value.range
            ...
            : -1;
    // As noted in the TypeCastExpression comments above, we're able to use a
    // normal whitespace regex here because we know for sure that this is a type
    // definition.
    const commentSyntax =
        commentStart >= 0 &&
  ...
  case "TypeParameter": {
    ...
    if (parent.params && parent.params.length === 1 && isTSXFile(options) &&
        !n.constraint && grandParent.type === "ArrowFunctionExpression") {
      parts.push(",");
    }
  ...
  case "TSIndexSignature": {
    ...
    const parametersGroup = group(concat([
      indent(concat([
        softline,
  ...
  case "TSTypePredicate":
    return concat([
      ...
      n.typeAnnotation ? concat([ " is ", path.call(print, "typeAnnotation") ])
                       : "",
    ]);
  ...
  case "TSMappedType": {
    const shouldBreak = hasNewlineInRange(
        options.originalText, options.locStart(n), options.locEnd(n));
    return group(
        concat([
          "{",
          indent(concat([
  ...
  case "TSMethodSignature":
    parts.push(n.accessibility ? concat([ n.accessibility, " " ]) : "",
               n.export ? "export " : "", n.static ? "static " : "",
               n.readonly ? "readonly " : "", n.computed ? "[" : "",
               path.call(print, "key"), n.computed ? "]" : "",
               printOptionalToken(path),
               printFunctionParams(path, print, options,
                                   /* expandArg */ false,
                                   /* printTypeParams */ true));
  ...
  case "TSImportEqualsDeclaration":
    ...
    parts.push("import ", path.call(print, "id"), " = ",
  ...
  case "TSModuleDeclaration": {
    ...
    if (parentIsDeclaration) {
      ...
      const textBetweenNodeAndItsId = options.originalText.slice(
          options.locStart(n), options.locStart(n.id));

      // Global declaration looks like this:
      // (declare)? global { ... }
      const isGlobalDeclaration =
          n.id.type === "Identifier" && n.id.name === "global" &&
  ...
  // TODO: Temporary auto-generated node type. To remove when typescript-estree
  // has proper support for private fields.
  case "TSPrivateIdentifier":
    return n.escapedText;

  case "TSConditionalType":
    return printTernaryOperator(path, options, print, {
      beforeParts : () => [path.call(print, "checkType"),
                           " ",
                           "extends",
                           " ",
  ...
  case "NGMicrosyntax":
    return concat(
        path.map((childPath, index) => concat([
                   index === 0 ? ""
  ...
  case "NGMicrosyntaxKeyedExpression": {
    ...
    const shouldNotPrintColon =
        ...
        (((index === 1 && (n.key.name === "then" || n.key.name === "else")) ||
          (index === 2 && n.key.name === "else" &&
           parentNode.body[index - 1].type === "NGMicrosyntaxKeyedExpression" &&
  ...
  // These are not valid TypeScript. Printing them just for the sake of error
  // recovery.
  case "TSJSDocAllType":
    return "*";
  ...
  case "TSJSDocFunctionType":
    return concat([
      "function(",
      // The parameters could be here, but typescript-estree doesn't convert
      // them anyway (throws an error).
      "): ",
      path.call(print, "typeAnnotation"),
    ]);
...
function printStatementSequence(path, options, print) {
  ...
  path.map((stmtPath, i) => {
    ...
    if (isNextLineEmpty(text, stmt, options.locEnd) &&
...
function printPropertyKey(path, options, print) {
  ...
  if (options.quoteProps === "consistent" && !needsQuoteProps.has(parent)) {
    const objectHasStringProp =
        (parent.properties || parent.body || parent.members)
            .some((prop) =>
                      !prop.computed && prop.key && isStringLiteral(prop.key) &&
  ...
  if (key.type === "Identifier" &&
    ...
    return path.call((keyPath) =>
                         comments.printComments(keyPath, () => prop, options),
                     "key");
  }

  if (isStringPropSafeToCoerceToIdentifier(node, options) &&
...
function printMethod(path, options, print) {
  ...
  parts.push(printPropertyKey(path, options, print),
             ...
             node === value ? printMethodInternal(path, options, print)
                            : path.call((path) => printMethodInternal(
                                            path, options, print),
                                        "value"));
...
function couldGroupArg(arg) {
  return ((arg.type === "ObjectExpression" &&
          ...
          (arg.type === "ArrowFunctionExpression" &&
           // we want to avoid breaking inside composite return types but not
           // simple keywords https://github.com/prettier/prettier/issues/4070
           ...
           // });
           (!arg.returnType || !arg.returnType.typeAnnotation ||
           ...
           (arg.body.type === "BlockStatement" ||
            ...
            arg.body.type === "ConditionalExpression" || isJSXNode(arg.body))));
}

function shouldGroupLastArg(args) {
  const lastArg = getLast(args);
  const penultimateArg = getPenultimate(args);
  return (!hasLeadingComment(lastArg) && !hasTrailingComment(lastArg) &&
...
function printJestEachTemplateLiteral(node, expressions, options) {
  ...
  if (headerNames.length > 1 ||
    ...
    const parts = [];
    const stringifiedExpressions =
        expressions.map((doc) => "${" + printDocToString(doc, {
                                          ...options,
                                          printWidth : Infinity,
                                          endOfLine : "lf",
                                        }).formatted +
    ...
    const maxColumnCount = Math.max(
        headerNames.length, ...tableBody.map((row) => row.cells.length));
    ...
    for (const {cells} of table.filter((row) => !row.hasLineBreak)) {
      cells.forEach((cell, index) => {
        maxColumnWidths[index] =
            Math.max(maxColumnWidths[index], getStringWidth(cell));
      });
    }

    parts.push(
        lineSuffixBoundary, "`", indent(concat([
          hardline,
          join(hardline,
               table.map((row) => join(
                             " | ",
...
function printArgumentsList(path, options, print) {
  ...
  if (args.length === 2 && args[0].type === "ArrowFunctionExpression" &&
      args[0].params.length === 0 && args[0].body.type === "BlockStatement" &&
      args[1].type === "ArrayExpression" && !args.find((arg) => arg.comments)) {
    return concat([
      "(",
  ...
  function shouldBreakForArrowFunctionInArguments(arg, argPath) {
    if (!arg || arg.type !== "ArrowFunctionExpression" || !arg.body ||
        arg.body.type !== "BlockStatement" || !arg.params ||
        arg.params.length < 1) {
      return false;
    }
  ...
  const printedArguments = path.map((argPath, index) => {
    ...
    shouldBreakForArrowFunction =
        shouldBreakForArrowFunctionInArguments(arg, argPath);
  ...
  if (path.getParentNode().type !== "Decorator" &&
  ...
  if (shouldGroupFirst || shouldGroupLast) {
    const shouldBreak =
        (shouldGroupFirst ? printedArguments.slice(1).some(willBreak)
                          : printedArguments.slice(0, -1).some(willBreak)) ||
        anyArgEmptyLine || shouldBreakForArrowFunction;
    ...
    return concat([
      ...
      conditionalGroup(
          [
            !somePrintedArgumentsWillBreak && !node.typeArguments &&
                    !node.typeParameters
                ? simpleConcat
                : ifBreak(allArgsBrokenOut(), simpleConcat),
            shouldGroupFirst ? concat([
              "(",
...
function printTypeAnnotation(path, options, print) {
  ...
  const isDefinite = node.definite ||
                     (parentNode && parentNode.type === "VariableDeclarator" &&
                      parentNode.definite);
...
function printFunctionParams(path, print, options, expandArg, printTypeParams) {
  ...
  if (fun[paramsField]) {
    ...
    printed = path.map((childPath, index) => {
      ...
      } else if (isParametersInTestCall || shouldHugParameters ||
  ...
  // If the parent is a call with the first/last argument expansion and this is
  // the params of the first/last argument, we don't want the arguments to break
  // and instead want the whole expression to be on a new line.
  //
  // Good:                 Bad:
  ...
  //   c
  // }) {}
  const hasNotParameterDecorator =
      fun[paramsField].every((param) => !param.decorators);
  if (shouldHugParameters && hasNotParameterDecorator) {
    return concat([ typeParams, "(", concat(printed), ")" ]);
  }

  // don't break in specs, eg; `it("should maintain parens around done even when
  // long", (done) => {})`
  ...
  const isFlowShorthandWithOneArg =
      (isObjectTypePropertyAFunction(parent, options) ||
       ...
       parent.type === "TypeAlias" || parent.type === "UnionTypeAnnotation" ||
       parent.type === "TSUnionType" ||
       parent.type === "IntersectionTypeAnnotation" ||
       (parent.type === "FunctionTypeAnnotation" &&
        parent.returnType === fun)) &&
      fun[paramsField].length === 1 && fun[paramsField][0].name === null &&
      fun[paramsField][0].typeAnnotation && fun.typeParameters === null &&
...
function canPrintParamsWithoutParens(node) {
  return (node.params.length === 1 && !node.rest && !node.typeParameters &&
          !hasDanglingComments(node) && node.params[0].type === "Identifier" &&
          !node.params[0].typeAnnotation && !node.params[0].comments &&
          !node.params[0].optional && !node.predicate && !node.returnType);
}
...
function printFunctionDeclaration(path, print, options) {
  ...
  parts.push(printFunctionTypeParameters(path, options, print), group(concat([
               printFunctionParams(path, print, options),
               printReturnType(path, print, options),
             ])),
             n.body ? " " : "", path.call(print, "body"));
...
function printExportDeclaration(path, options, print) {
  ...
  if (decl.declaration) {
    ...
    if (isDefault && decl.declaration.type !== "ClassDeclaration" &&
    ...
    if (decl.specifiers && decl.specifiers.length > 0) {
      ...
      const canBreak =
          specifiers.length > 1 || defaultSpecifiers.length > 0 ||
          (decl.specifiers && decl.specifiers.some((node) => node.comments));

      let printed = "";
      if (specifiers.length !== 0) {
        if (canBreak) {
          printed = group(concat([
            "{",
            indent(concat([
              options.bracketSpacing ? line : softline,
      ...
      parts.push(decl.exportKind === "type" ? "type " : "",
                 ...
                 concat([ isNamespaceFollowed ? ", " : "" ]), printed);
    } else {
      parts.push("{}");
    }
...
function printTypeParameters(path, options, print, paramsKey) {
  ...
  const shouldInline =
      isParameterInTestCall || n[paramsKey].length === 0 ||
  ...
  function printDanglingCommentsForInline(n) {
    ...
    const hasOnlyBlockComments =
        n.comments.every(handleComments.isBlockComment);
    const printed =
        comments.printDanglingComments(path, options,
  ...
  return group(concat([
    ...
    ifBreak(options.parser !== "typescript" && options.parser !== "babel-ts" &&
                    shouldPrintComma(options, "all")
                ? ","
                : ""),
    softline,
...
function printClass(path, options, print) {
  ...
  if (n.superClass) {
    ...
    if ((!n.implements || n.implements.length === 0) &&
      ...
      parts.push(concat([
        " ",
        path.call((superClass) => comments.printComments(
                      superClass, () => printed, options),
                  "superClass"),
      ]));
    } else {
      partsGroup.push(group(concat([
        line,
        path.call((superClass) => comments.printComments(
                      superClass, () => printed, options),
                  "superClass"),
...
function printMemberLookup(path, options, print) {
  ...
  return group(concat([
    optional, "[", indent(concat([ softline, property ])), softline, "]"
  ]));
}
...
function printMemberChain(path, options, print) {
  ...
  function shouldInsertEmptyLineAfter(node) {
    ...
    const nextCharIndex = getNextNonSpaceNonCommentCharacterIndex(
        originalText, node, options.locEnd);
    const nextChar = originalText.charAt(nextCharIndex);

    // if it is cut off by a parenthesis, we only account for one typed empty
    // line after that parenthesis
    if (nextChar === ")") {
      return isNextLineEmptyAfterIndex(originalText, nextCharIndex + 1,
                                       options.locEnd);
    }
  ...
  for (; i < printedNodes.length; ++i) {
    if (printedNodes[i].node.type === "TSNonNullExpression" ||
  ...
  function isFactory(name) { return /^[A-Z]|^[_$]+$/.test(name); }
  ...
  const lastNodeBeforeIndent =
      getLast(shouldMerge ? groups.slice(1, 2)[0] : groups[0]).node;
  ...
  const callExpressions =
      printedNodes.map(({node}) => node).filter(isCallOrOptionalCallExpression);
  ...
  if (hasComment ||
      (callExpressions.length > 2 &&
       callExpressions.some((expr) => !expr.arguments.every(
                                (arg) => isSimpleCallArgument(arg, 0)))) ||
      ...
      (((lastGroupDoc, lastGroupNode) =>
            ...
            willBreak(lastGroupDoc))(getLast(printedGroups),
                                     getLast(getLast(groups)).node) &&
       callExpressions.slice(0, -1).some(
           (n) => n.arguments.some(isFunctionOrArrowExpression)))) {
...
function separatorNoWhitespace(isFacebookTranslationTag, child, childNode,
                               nextNode) {
  if (isFacebookTranslationTag) {
    return "";
  }

  if ((childNode.type === "JSXElement" && !childNode.closingElement) ||
      (nextNode && nextNode.type === "JSXElement" &&
...
function separatorWithWhitespace(isFacebookTranslationTag, child, childNode,
                                 nextNode) {
  if (isFacebookTranslationTag) {
    return hardline;
  }
  ...
  path.map((childPath, i) => {
    ...
    if (isLiteral(child)) {
      ...
      if (isMeaningfulJSXText(child)) {
        ...
        if (words[0] === "") {
          ...
          if (/\n/.test(words[0])) {
            ...
            children.push(separatorWithWhitespace(isFacebookTranslationTag,
                                                  words[1], child, next));
          } else {
        ...
        if (endWhitespace !== undefined) {
          if (/\n/.test(endWhitespace)) {
            const next = n.children[i + 1];
            children.push(separatorWithWhitespace(
                isFacebookTranslationTag, getLast(children), child, next));
          } else {
            children.push(jsxWhitespace);
          }
        } else {
          const next = n.children[i + 1];
          children.push(separatorNoWhitespace(isFacebookTranslationTag,
                                              getLast(children), child, next));
        }
      } else if (/\n/.test(text)) {
      ...
      if (directlyFollowedByMeaningfulText) {
        const firstWord =
            rawText(next).trim().split(matchJsxWhitespaceRegex)[0];
...
function printJSXElement(path, options, print) {
  ...
  const openingLines = n.type === "JSXElement"
                           ? path.call(print, "openingElement")
                           : path.call(print, "openingFragment");
  const closingLines = n.type === "JSXElement"
                           ? path.call(print, "closingElement")
  ...
  let forcedBreak = willBreak(openingLines) || containsTag ||
                    containsMultipleAttributes || containsMultipleExpressions;
  ...
  const isFacebookTranslationTag = n.openingElement && n.openingElement.name &&
  ...
  for (let i = children.length - 2; i >= 0; i--) {
    ...
    const isPairOfHardlines = children[i] === hardline &&
                              children[i + 1] === "" &&
                              children[i + 2] === hardline;
    const isLineFollowedByJSXWhitespace =
        (children[i] === softline || children[i] === hardline) &&
        children[i + 1] === "" && children[i + 2] === jsxWhitespace;
    const isJSXWhitespaceFollowedByLine =
        children[i] === jsxWhitespace && children[i + 1] === "" &&
        (children[i + 2] === softline || children[i + 2] === hardline);
    const isDoubleJSXWhitespace = children[i] === jsxWhitespace &&
                                  children[i + 1] === "" &&
                                  children[i + 2] === jsxWhitespace;
    const isPairOfHardOrSoftLines =
        (children[i] === softline && children[i + 1] === "" &&
         children[i + 2] === hardline) ||
        (children[i] === hardline && children[i + 1] === "" &&
         children[i + 2] === softline);

    if ((isPairOfHardlines && containsText) || isPairOfEmptyStrings ||
        isLineFollowedByJSXWhitespace || isDoubleJSXWhitespace ||
...
function maybeWrapJSXElementInParens(path, elem, options) {
  ...
  const shouldBreak =
      path.match(undefined, (node) => node.type === "ArrowFunctionExpression",
                 isCallOrOptionalCallExpression,
...
function printBinaryishExpressions(path, print, options, isNested,
  ...
  if (isBinaryish(node)) {
    ...
    if (shouldFlatten(node.operator, node.left.operator)) {
      ...
      parts =
          parts.concat(path.call((left) => printBinaryishExpressions(
                                     left, print, options,
    ...
    const lineBeforeOperator =
        (node.operator === "|>" || node.type === "NGPipeExpression" ||
...
function printAssignmentRight(leftNode, rightNode, printedRight, options) {
  ...
  const canBreak =
      ...
      (rightNode.type === "ClassExpression" && rightNode.decorators &&
       rightNode.decorators.length) ||
      ((leftNode.type === "Identifier" || isStringLiteral(leftNode) ||
...
function printAssignment(leftNode, printedLeft, operator, rightNode,
                         printedRight, options) {
  if (!rightNode) {
    return printedLeft;
  }

  const printed =
      printAssignmentRight(leftNode, rightNode, printedRight, options);
...
function exprNeedsASIProtection(path, options) {
  ...
  const maybeASIProblem =
      ...
      node.type === "TemplateLiteral" || node.type === "TemplateElement" ||
      isJSXNode(node) || (node.type === "BindExpression" && !node.object) ||
  ...
  return path.call((childPath) => exprNeedsASIProtection(childPath, options),
...
function shouldHugType(node) {
  ...
  if (node.type === "UnionTypeAnnotation" || node.type === "TSUnionType") {
    const voidCount =
        node.types
            .filter((n) => n.type === "VoidTypeAnnotation" ||
...
function willPrintOwnComments(path /*, options */) {
  ...
  return (
      ((node && (isJSXNode(node) || hasFlowShorthandAnnotationComment(node) ||
       ...
       (parent && (parent.type === "JSXSpreadAttribute" ||
      ...
      (!hasIgnoreComment(path) || parent.type === "UnionTypeAnnotation" ||
       parent.type === "TSUnionType"));
}

function canAttachComment(node) {
  return (node.type && node.type !== "CommentBlock" &&
          node.type !== "CommentLine" && node.type !== "Line" &&
          node.type !== "Block" && node.type !== "EmptyStatement" &&
          node.type !== "TemplateElement" && node.type !== "Import");
}
...
function printComment(commentPath, options) {
  ...
  case "Line":
    ...
    if (options.originalText.slice(options.locStart(comment))
            .startsWith("#!")) {
...
function printIndentableBlockComment(comment) {
  ...
  return concat([
    "/*",
    join(hardline, lines.map((line, index) =>
                                 index === 0 ? line.trimEnd()
