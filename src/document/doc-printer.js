...
function rootIndent() { return {value : "", length : 0, queue : []}; }
...
function makeAlign(ind, n, options) {
  return n === -Infinity
             ? ind.root || rootIndent()
             : n < 0 ? generateInd(ind, {type : "dedent"}, options)
                     : !n ? ind
                          : n.type === "root"
                                ? {...ind, root : ind}
                                : typeof n === "string"
...
function trim(out) {
  ...
  while (out.length > 0 && typeof out[out.length - 1] === "string" &&
...
function printDocToString(doc, options) {
  ...
  while (cmds.length !== 0) {
    ...
    if (typeof doc === "string") {
      ...
      case "fill": {
        ...
        const firstAndSecondContentFits =
            fits(firstAndSecondContentFlatCmd, [], rem, options, true);
  ...
  if (cursorPlaceholderIndex !== -1) {
    const otherCursorPlaceholderIndex =
        out.indexOf(cursor.placeholder, cursorPlaceholderIndex + 1);
    const beforeCursor = out.slice(0, cursorPlaceholderIndex).join("");
    const aroundCursor =
        out.slice(cursorPlaceholderIndex + 1, otherCursorPlaceholderIndex)
            .join("");
