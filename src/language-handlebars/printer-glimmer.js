...
function print(path, options, print) {
  ...
  if (hasPrettierIgnore(path)) {
    const startOffset = locationToOffset(
        options.originalText, n.loc.start.line - 1, n.loc.start.column);
    const endOffset = locationToOffset(options.originalText, n.loc.end.line - 1,
                                       n.loc.end.column);
  ...
  case "ElementNode": {
    ...
    const hasNonWhitespaceChildren =
        n.children.some((n) => !isWhitespaceNode(n));

    const isVoid = (isGlimmerComponent(n) &&
    ...
    const printParams = (path, print) => indent(concat([
    ...
    return concat([
      ...
      !isVoid ? group(concat([
        hasNonWhitespaceChildren ? indent(printChildren(path, options, print))
                                 : "",
  ...
  case "BlockStatement": {
    ...
    const isElseIf = pp && pp.inverse && pp.inverse.body.length === 1 &&
                     pp.inverse.body[0] === n &&
                     pp.inverse.body[0].path.parts[0] === "if";
    const hasElseIf = n.inverse && n.inverse.body.length === 1 &&
                      n.inverse.body[0].type === "BlockStatement" &&
                      n.inverse.body[0].path.parts[0] === "if";
    const indentElse = hasElseIf ? (a) => a : indent;
    const inverseElseStatement = (n.inverseStrip.open ? "{{~" : "{{") + "else" +
    ...
    if (n.inverse) {
      return concat([
        ...
        n.inverse && !hasElseIf ? concat([ hardline, inverseElseStatement ])
                                : "",
        n.inverse
    ...
    const hasNonWhitespaceChildren =
        n.program.body.some((n) => !isWhitespaceNode(n));
  ...
  case "SubExpression": {
    ...
    const printedParams =
        params.length > 0 ? indent(concat([ line, group(join(line, params)) ]))
                          : "";
    return group(
  ...
  case "AttrNode": {
    ...
    const quotedValue =
        isText ? printStringLiteral(value.parts.join(), options) : value;
  ...
  case "TextNode": {
    ...
    if ((isFirstElement || isLastElement) && isWhitespaceOnly &&
    ...
    if (isWhitespaceOnly && lineBreaksCount) {
      leadingLineBreaksCount =
          Math.min(lineBreaksCount, maxLineBreaksToPreserve);
      ...
      if (isPreviousNodeOfSomeType(path, [ "ElementNode" ]) ||
    ...
    if (inAttrNode) {
      ...
      if (trailingLineBreaksCount === 0 &&
          isNextNodeOfSomeType(path, [ "MustacheStatement" ])) {
        trailingSpace = " ";
      }

      if (leadingLineBreaksCount === 0 &&
    ...
    return concat([
      ...
      n.chars.replace(/^[\s ]+/g, leadingSpace)
...
function printStringLiteral(stringLiteral, options) {
  ...
  if (stringLiteral.includes(preferred.quote) ||
      stringLiteral.includes(alternate.quote)) {
    const numPreferredQuotes =
        (stringLiteral.match(preferred.regex) || []).length;
    const numAlternateQuotes =
        (stringLiteral.match(alternate.regex) || []).length;

    shouldUseAlternateQuote = numPreferredQuotes > numAlternateQuotes;
  }

  const enclosingQuote = shouldUseAlternateQuote ? alternate : preferred;
  const escapedStringLiteral =
      stringLiteral.replace(enclosingQuote.regex, `\\${enclosingQuote.quote}`);
