function formatMarkdown(input, output, output2, version, url, options,
                        cliOptions, full) {
...
function codeBlock(content, syntax) {
  ...
  const longestBacktickSequenceLength =
      Math.max(...backtickSequences.map(({length}) => length));
