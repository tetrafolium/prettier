...
const postFile = path.join(
    blogDir, `${new Date().toISOString().replace(/T.+/, "")}-${version}.md`);
...
const dirs = fs.readdirSync(changelogUnreleasedDir, {
                 withFileTypes : true
...
for (const dir of dirs) {
  ...
  category.entries =
      fs.readdirSync(path.join(changelogUnreleasedDir, dir.name))
          ...
          .map((fileName) => {
            ...
            return {
              ...
              content : [
                title.replace(/\[(BREAKING|HIGHLIGHT)\]/g, "")
                    .replace(/\s+/g, " ")
