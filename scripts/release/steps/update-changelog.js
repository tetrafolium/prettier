...
function writeChangelog({version, previousVersion, releaseNotes}) {
  ...
  const newEntry = dedent`
    ...
    [diff](https://github.com/prettier/prettier/compare/${previousVersion}...${
      version})
...
module.exports = async function({version, previousVersion}) {
  ...
  if (semverDiff !== "patch") {
    ...
    console.warn(dedent(chalk`
        {yellow warning} The file {bold ${
        blogPost
            .file}} doesn't exist, but it will be referenced in {bold CHANGELOG.md}. Make sure to create it later.
