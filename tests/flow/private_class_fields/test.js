...
class IncompatibleGetAndSet {
  #p: number static #q: number
  constructor() {
    const a: number = 3;
    a.#p = 3;                            // Error, #p not on number
    const b: {p: number} = {p : 3} b.#p; // Error, #p not on b.
  }
}

class Static {
  static #p: number static #q: number
  #r: number
  m(): number {
    Static.#p = 2;
...
class Annotations {
  ...
  test1(): number { return this.#p; }
  test2(): number {
    return this.#t; // Error, string ~> number
  }
  test3(): number { return Annotations.#sp; }
  test4(): number {
    return Annotations.#st; // Error, string ~> number
  }
  test5(): number { return this.#r; }
  test6(): string {
    return this.#r; // Error, number ~> string
  }
  test5And6Helper() { this.#r = 3; }
  test7(): number { return Annotations.#sr; }
  test8(): string {
    return Annotations.#sr; // Error, number ~> string
  }
  test7And9Helper() { Annotations.#sr = 3; }
}
...
class RefinementClashes {
  ...
  test2() {
    if (RefinementClashes.#q === 3) {
      (RefinementClashes.q:
           3); // Error, RefinementClashes.#q doesn't refine RefinementClashes.q
      (RefinementClashes.#q: 3);
      RefinementClashes.q = 4;
      (RefinementClashes.q: 4);
      (RefinementClashes.#q:
           4); // Error, RefinementClashes.q doesnt refine RefinementClashes.#q
      (RefinementClashes.#q:
           3); // RefinementClashes.q doesnt havoc RefinementClashes.#q
      RefinementClashes.#q = 3;
      (RefinementClashes.q:
           3); // Error, RefinementClashes.#q doesnt refine RefinementClashes.q
      (RefinementClashes.q:
           4); // RefinementClashes.#q doesnt havoc RefinementClashes.q
    }
