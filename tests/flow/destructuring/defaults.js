...
function obj_prop_fun({p : {q = 0} = { q: true }} = {
  p : {q : ""}
}) {
  // errors:
...
function obj_rest({p : {q, ...o} = { q: 0, r: 0 }} = {
  p : {q : 0, r : ""}
}) {
  // errors:
...
function gen<T>(x: T, {p = x}: {p: T}): T { return p; }
...
function obj_prop_opt({p}: {p?: string} = {
  p : 0
}) {}
function obj_prop_maybe({p}:{p:?string}={p:0}) {}
function obj_prop_union({p}: {p: number|string} = {
  p : true
}) {}

// TODO: union-of-objects upper bounds preserved through destructuring
function obj_prop_union2({p}: {p: number}|{p : string} = {
  p : true
}) {}
