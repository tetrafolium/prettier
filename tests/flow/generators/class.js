class GeneratorExamples {
  ...
  * delegate_next_iterable(xs: Array<number>) { yield* xs; }

  * delegate_yield_iterable(xs: Array<number>) { yield* xs; }

  * delegate_return_iterable(xs: Array<number>) {
    var x: void = yield* xs // ok: Iterator has no yield value
  }

  * generic_yield<Y>(y: Y): Generator<Y, void, void> { yield y; }

  * generic_return<R>(r: R): Generator<void, R, void> { return r; }

  * generic_next<N>(): Generator<void, N, N> { return yield undefined; }
}
...
var infer_stmt_next =
    examples.infer_stmt().next(0).value; // error: number ~> boolean
