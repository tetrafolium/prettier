...
// If each type in U has construct signatures and the sets of construct
// signatures are identical ignoring return types, U has the same set of
// construct signatures, but with return types that are unions of the return
// types of the respective construct signatures from each type in U.
var unionOfDifferentReturnType: {new (a: number): number;}|{
  new (a: number): Date;
};
...
var unionOfDifferentParameterTypes: {new (a: number): number;}|{
  new (a: string): Date;
};
...
var unionOfDifferentNumberOfSignatures: {new (a: number): number;}|{
  new (a: number): Date;
  new (a: string): boolean;
};
...
var unionWithDifferentParameterCount: {new (a: string): string;}|{
...
strOrNum =
    new unionWithRestParameter1('hello', "hello"); // error in parameter type
strOrNum = new unionWithRestParameter1();          // error

var unionWithRestParameter2: {new (a: string, ...b: number[]): string;}|
    {new (a: string, b: number) : number};
...
strOrNum =
    new unionWithRestParameter2('hello', 10, 11); // error no call signature
strOrNum =
    new unionWithRestParameter2('hello', "hello"); // error no call signature
strOrNum = new unionWithRestParameter2();          // error no call signature

var unionWithRestParameter3: {new (a: string, ...b: number[]): string;}|
    {new (a: string) : number};
...
strOrNum =
    new unionWithRestParameter3('hello', 10, 11); // error no call signature
strOrNum =
    new unionWithRestParameter3('hello', "hello"); // error no call signature
