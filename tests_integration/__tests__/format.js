...
test(
    "typescript parser should throw the first error when both JSX and non-JSX mode failed",
    () => {
      const input = `
...
test("should work with foo plugin instance", () => {
  const input = "a:\r\n  123\r\n";
  expect(JSON.stringify(prettier.format(
             input, {parser : "foo-parser", plugins : [ fooPlugin ]})))
