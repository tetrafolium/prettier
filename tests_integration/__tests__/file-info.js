...
describe(
    "extracts file-info with ignored=true for a file in a hand-picked .prettierignore",
    () => {
      runPrettier("cli/", [
        "--file-info",
        "regular-module.js",
...
describe(
    "extracts file-info with ignored=false for a file in node_modules when --with-node-modules provided",
    () => {
      runPrettier("cli/with-node-modules/", [
        "--file-info",
        "node_modules/file.js",
...
describe(
    "extracts file-info with inferredParser=foo when plugins are autoloaded",
    () => {
...
describe(
    "extracts file-info with inferredParser=foo when plugins are loaded with --plugin-search-dir",
    () => {
      runPrettier("cli/", [
        "--file-info",
        "file.foo",
...
describe(
    "extracts file-info with inferredParser=foo when a plugin is hand-picked",
    () => {
      runPrettier("cli/", [
        "--file-info",
        "file.foo",
...
test("API getFileInfo with resolveConfig", () => {
  const file1 =
      path.resolve(path.join(__dirname, "../cli/with-resolve-config/file.foo"));
  const file2 =
      path.resolve(path.join(__dirname, "../cli/with-resolve-config/file.bar"));
  ...
  expect(prettier.getFileInfo(file1, {
    resolveConfig : true,
  })).resolves.toMatchObject({
    ignored : false,
    inferredParser : "json",
  });
  expect(prettier.getFileInfo(file2, {
    resolveConfig : true,
  })).resolves.toMatchObject({
    ignored : false,
...
test("API getFileInfo.sync with resolveConfig", () => {
  ...
  expect(prettier.getFileInfo.sync(file1, {
    resolveConfig : true,
  })).toMatchObject({
    ignored : false,
    inferredParser : "json",
  });
  expect(prettier.getFileInfo.sync(file2, {
    resolveConfig : true,
  })).toMatchObject({
    ignored : false,
    inferredParser : "babel",
...
test("API getFileInfo.sync with resolveConfig when no config is present",
     () => {
       const file1 = path.resolve(path.join(
           __dirname, "../cli/with-resolve-config-no-config/file.foo"));
       const file2 = path.resolve(path.join(
           __dirname, "../cli/with-resolve-config-no-config/file.bar"));
...
test("API getFileInfo with ignorePath", () => {
  ...
  const ignorePath =
      path.resolve(path.join(__dirname, "../cli/ignore-path/.prettierignore"));
  ...
  expect(prettier.getFileInfo(file, {
    ignorePath,
  })).resolves.toMatchObject({
    ignored : true,
    inferredParser : "babel",
...
test("API getFileInfo.sync with ignorePath", () => {
  ...
  expect(prettier.getFileInfo.sync(file, {
    ignorePath,
  })).toMatchObject({
    ignored : true,
    inferredParser : "babel",
...
describe("API getFileInfo.sync with ignorePath", () => {
  ...
  test("with relative filePath", () => {
    expect(prettier.getFileInfo.sync(filePath, options).ignored)
  ...
  test("with absolute filePath", () => {
    expect(prettier.getFileInfo.sync(path.resolve(filePath), options).ignored)
        .toMatchInlineSnapshot("true");
  });
});
...
describe(
    "extracts file-info for a JS file with no extension but a standard shebang",
    () => {
      expect(prettier.getFileInfo.sync(
                 "tests_integration/cli/shebang/node-shebang"))
          .toMatchObject({
            ignored : false,
            inferredParser : "babel",
          });
...
describe(
    "extracts file-info for a JS file with no extension but an env-based shebang",
    () => {
      expect(prettier.getFileInfo.sync(
                 "tests_integration/cli/shebang/env-node-shebang"))
          .toMatchObject({
            ignored : false,
            inferredParser : "babel",
          });
...
describe("returns null parser for unknown shebang", () => {
  expect(prettier.getFileInfo.sync(
             "tests_integration/cli/shebang/nonsense-shebang"))
      .toMatchObject({
        ignored : false,
        inferredParser : null,
      });
...
test("API getFileInfo with plugins loaded using pluginSearchDir", () => {
  const file = "file.foo";
  const pluginsPath =
      path.resolve(path.join(__dirname, "../plugins/automatic"));
  expect(prettier.getFileInfo(file)).resolves.toMatchObject({
    ignored : false,
    inferredParser : null,
  });
...
test("API getFileInfo with hand-picked plugins", () => {
  const file = "file.foo";
  const pluginPath = path.resolve(path.join(
      __dirname, "../plugins/automatic/node_modules/@prettier/plugin-foo"));
  expect(prettier.getFileInfo(file)).resolves.toMatchObject({
    ignored : false,
    inferredParser : null,
  });
