...
describe(
    "CLI overrides are still applied when no config is found with --config-precedence file-override",
    () => {
...
describe(
    "CLI overrides gets applied when no config exists with --config-precedence prefer-file",
    () => {
      runPrettier("cli/config/no-config/", [
        "--end-of-line",
        "lf",
