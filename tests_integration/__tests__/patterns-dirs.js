...
//
// 1. `prettier dir1 dir2` – prettify all files with supported extensions inside
// `dir1` and `dir2`.
//
// 2. `prettier dir1 "dir2/**/*"` – prettify all files with supported extensions
// inside `dir1`
//     as well as all files matched by the `dir2/**/*` glob.
//     If any of the latter files have unknown extensions – log an error for
//     them. (*)
//
// 3. `prettier non-exists-dir "dir2/**/*""` – log an error that
// `non-exists-dir` resulted in 0 files
//     and prettify all files matched by the `dir2/**/*` glob.
//     If any of the latter files have unknown extensions – log an error for
//     them. (*) (Note: ESLint just prints an error and doesn't process
//     anything.)
//
...
const fs = require("fs");
if (path.sep === "/") {
  // Don't use snapshots in these tests as they're conditionally executed on
  // non-Windows only.
  ...
  // `runPrettier` should be refactored to use `describe` and `beforeEach` for
  // doing setup.
